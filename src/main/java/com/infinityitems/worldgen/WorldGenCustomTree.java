...
public class WorldGenCustomTree  extends WorldGenerator {
    ...
    public WorldGenCustomTree(Block log, Block leaves, boolean notify) {
        super(notify);
        this.logBlock = log;
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int trunkHeight = 0; trunkHeight < height; trunkHeight++) {
            world.setBlock(x, y + trunkHeight, z, logBlock, 0, 2);  // Установка блока ствола
        }
                ...
                for (int leafZ = z - radius; leafZ <= z + radius; leafZ++) {
                    ...
                    if (distance <= radius) {  // Генерация листвы только внутри радиуса
                        if (world.isAirBlock(leafX, yOffset, leafZ) || world.getBlock(leafX, yOffset, leafZ).isLeaves(world, leafX, yOffset, leafZ)) {
                            world.setBlock(leafX, yOffset, leafZ, leavesBlock, 0, 2);  // Установка листвы
                        }
                    }
                }
            }
